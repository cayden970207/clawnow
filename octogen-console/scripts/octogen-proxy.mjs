...
const COOKIE_PREFIX = "octogen_tp_";
...
function buildForwardHeaders(req, upstream, claims) {
  ...
  headers["X-Octogen-Verified"] = "1";
  headers["X-Octogen-Instance-Id"] = claims.instanceId;
  headers["X-Octogen-Session-Type"] = claims.sessionType;
...
async function writeHealthResponse(res, config) {
  ...
  const desktop = desktopTarget
    ? await checkTcpConnectivity({ ...desktopTarget, timeoutMs: 700 })
    : null;

  const success = openclaw.ok && (desktop ? desktop.ok : true);
  const payload = {
    success,
    service: "octogen-proxy",
...
function enforceGatewayTrustedProxyConfig(config) {
  ...
  // Octogen Console runs in managed trusted-proxy mode. Keep gateway auth aligned with
  // console-issued trusted-proxy sessions to avoid auth-mode drift.
  ...
  // Octogen Console proxy is the trust boundary for managed browser sessions.
...
async function handleGatewayRepair(req, res, config) {
  ...
  res.statusCode = 200;
  res.setHeader("content-type", "application/json; charset=utf-8");
  res.end(JSON.stringify({ success: true, service: "octogen-proxy", repaired: true, details }));
}
...
async function handleWorkspaceReleaseSync(req, res, config) {
  ...
  if (!manifestUrl) {
    writeHttpError(
      ...
      400,
      "manifest_missing",
      "Workspace release manifest URL missing (provide manifestUrl or set OCTOGEN_CONTROL_UI_MANIFEST_URL)",
    );
    return;
  }
...
function buildTargetPath(route, requestUrl) {
  const url = new URL(requestUrl, "http://octogen.local");
...
function createProxyConfig() {
  ...
  const sharedSecret = process.env.OCTOGEN_PROXY_SHARED_SECRET?.trim();
  if (!sharedSecret) {
    throw new Error("OCTOGEN_PROXY_SHARED_SECRET is required");
  }

  return {
    bind: process.env.OCTOGEN_PROXY_BIND?.trim() || "127.0.0.1",
    port: parsePort(process.env.OCTOGEN_PROXY_PORT, 18790),
    sharedSecret,
    tokenClockSkewSeconds: parsePositiveInt(
      process.env.OCTOGEN_PROXY_TOKEN_CLOCK_SKEW_SECONDS,
      120,
    ),
    expectedIss: process.env.OCTOGEN_PROXY_EXPECTED_ISS?.trim() || "octogen-console",
    expectedAud: process.env.OCTOGEN_PROXY_EXPECTED_AUD?.trim() || "openclaw-gateway",
    instanceId:
      process.env.OCTOGEN_VM_ID?.trim() || process.env.OCTOGEN_INSTANCE_ID?.trim() || "local",
    controlPrefix: normalizePrefix(process.env.OCTOGEN_PROXY_CONTROL_PREFIX, "/octogen"),
    desktopPrefix: normalizePrefix(
      process.env.OCTOGEN_PROXY_DESKTOP_PREFIX || process.env.OCTOGEN_PROXY_NOVNC_PREFIX,
      "/desktop",
    ),
    /** @deprecated Use desktopPrefix. */
    novncPrefix: normalizePrefix(process.env.OCTOGEN_PROXY_NOVNC_PREFIX, "/novnc"),
    localGatewayPrefix: normalizePrefix(
      process.env.OCTOGEN_PROXY_LOCAL_GATEWAY_PREFIX,
      "/__octogen/local-gateway",
    ),
    healthPath: normalizePrefix(process.env.OCTOGEN_PROXY_HEALTH_PATH, "/__octogen/health"),
    bootstrapPath: normalizePrefix(
      process.env.OCTOGEN_PROXY_BOOTSTRAP_PATH,
      "/__octogen/bootstrap",
    ),
    repairPrefix: normalizePrefix(process.env.OCTOGEN_PROXY_REPAIR_PREFIX, "/__octogen/repair"),
    controlUiManifestUrl: process.env.OCTOGEN_CONTROL_UI_MANIFEST_URL?.trim() || "",
    controlUiUpdaterScriptPath:
      process.env.OCTOGEN_CONTROL_UI_UPDATER_SCRIPT_PATH?.trim() ||
      "/opt/octogen/octogen-control-ui-updater.sh",
    controlUiUpdaterScriptUrl: process.env.OCTOGEN_CONTROL_UI_UPDATER_SCRIPT_URL?.trim() || "",
    controlUiInstallRoot:
      process.env.OCTOGEN_CONTROL_UI_INSTALL_ROOT?.trim() || "/opt/octogen/control-ui",
    bootstrapStateFile:
      process.env.OCTOGEN_BOOTSTRAP_STATE_FILE?.trim() || "/var/lib/octogen/bootstrap-state.json",
    stripControlPrefix: parseBoolean(process.env.OCTOGEN_PROXY_STRIP_CONTROL_PREFIX, false),
    stripDesktopPrefix: parseBoolean(
      process.env.OCTOGEN_PROXY_STRIP_DESKTOP_PREFIX ??
        process.env.OCTOGEN_PROXY_STRIP_NOVNC_PREFIX,
      true,
    ),
    openclawUpstream: parseUpstream("OCTOGEN_OPENCLAW_UPSTREAM", "http://127.0.0.1:18789"),
    trustedProxyUser: process.env.OCTOGEN_TRUSTED_PROXY_USER?.trim() || "",
    openclawLocalAddress: parseLoopbackLocalAddress(
      process.env.OCTOGEN_OPENCLAW_LOCAL_ADDRESS,
      "127.0.0.2",
    ),
    openclawService: process.env.OCTOGEN_OPENCLAW_SERVICE?.trim() || "openclaw-gateway.service",
    desktopUpstream: parseUpstream(
      "OCTOGEN_DESKTOP_UPSTREAM",
      parseUpstream("OCTOGEN_NOVNC_UPSTREAM", "http://127.0.0.1:8080"),
    ),
  };
}
...
function resolveRoute(config, pathname) {
  ...
  // Support the noVNC prefix as an alternate desktop viewer entrypoint.
...
function parseAndValidateToken(req, config, expectedSessionType) {
  const requestUrl = new URL(req.url || "/", "http://octogen.local");
...
function createServer(config) {
  const server = http.createServer((req, res) => {
    ...
    if (route.kind === "repair") {
      ...
      writeHttpError(res, 404, "route_not_found", "Unknown Octogen Console repair route");
      return;
    }

    if (route.kind === "missing-desktop") {
      writeHttpError(res, 503, "desktop_unavailable", "OCTOGEN_DESKTOP_UPSTREAM is not configured");
      return;
    }

    if (route.kind === "unknown") {
      writeHttpError(res, 404, "route_not_found", "Unknown Octogen Console proxy route");
      return;
    }
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    if (route.kind !== "proxy") {
      writeUpgradeError(socket, 404, "Unknown Octogen Console proxy route");
      return;
    }
    try {
      const auth = resolveRouteAuth(req, config, route);
...
function main() {
  ...
  server.listen(config.port, config.bind, () => {
    ...
    console.log(
      `[octogen-proxy] listening on http://${config.bind}:${config.port}\n` +
