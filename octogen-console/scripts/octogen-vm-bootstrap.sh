...
usage() {
  cat <<'USAGE'
Usage: octogen-vm-bootstrap.sh --proxy-secret <secret> [options]

Required:
  --proxy-secret <secret>      Shared secret for Octogen Console trusted-proxy tokens
...
Optional:
  ...
  --control-prefix <path>      Control UI path prefix (default: /octogen)
  ...
  --vm-id <id>                 Octogen VM id (alias: --instance-id)
  ...
  --gateway-origin-template    Gateway URL template (for example: https://{{IPV4}}.sslip.io)
                               Supported placeholders: {{IPV4}}, {{VM_ID}}, {{INSTANCE_ID}}, {{SERVER_ID}}
  --control-ui-origin <origin> Explicit Control UI browser origin allowlist entry
  --console-device-id <id>     Pre-approved console device id for gateway admin automation
  --console-device-public-key <key>
                               Base64url public key for the pre-approved console device
  ...
  --proxy-script-url <url>     Proxy script URL
                               (default: https://raw.githubusercontent.com/cayden970207/octogen/main/octogen-console/scripts/octogen-proxy.mjs)
  --hermes-runner-script-url <url>
                               Hermes runner script URL
                               (default: https://raw.githubusercontent.com/cayden970207/octogen/main/octogen-console/scripts/octogen-hermes-runner.mjs)
  ...
  --control-ui-updater-script-url <url>
                               ...
                               (default: https://raw.githubusercontent.com/cayden970207/octogen/main/octogen-console/scripts/octogen-control-ui-updater.sh)
  --console-api-base-url <url> Octogen Console API base URL for VM-side runners

Logs:
  /var/log/octogen-bootstrap.log
  /var/lib/octogen/bootstrap-state.json
USAGE
}
...
PROXY_SCRIPT_URL="https://raw.githubusercontent.com/cayden970207/octogen/main/octogen-console/scripts/octogen-proxy.mjs"
HERMES_RUNNER_SCRIPT_URL="https://raw.githubusercontent.com/cayden970207/octogen/main/octogen-console/scripts/octogen-hermes-runner.mjs"
CONTROL_UI_MANIFEST_URL=""
CONTROL_UI_UPDATER_SCRIPT_URL="https://raw.githubusercontent.com/cayden970207/octogen/main/octogen-console/scripts/octogen-control-ui-updater.sh"
CONTROL_UI_ROOT=""
CONSOLE_DEVICE_ID=""
CONSOLE_DEVICE_PUBLIC_KEY=""
CONSOLE_API_BASE_URL=""

BOOTSTRAP_LOG="/var/log/octogen-bootstrap.log"
STATE_DIR="/var/lib/octogen"
...
DESKTOP_NEKO_CONTAINER="octogen-neko"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --console-device-id)
      ...
      shift 2
      ;;
    --console-device-public-key)
      CONSOLE_DEVICE_PUBLIC_KEY="${2:-}"
      shift 2
      ;;
    --disable-https)
      ENABLE_HTTPS="0"
...
if [[ -n "$CONSOLE_DEVICE_ID" && -z "$CONSOLE_DEVICE_PUBLIC_KEY" ]]; then
  echo "--console-device-public-key is required when --console-device-id is set" >&2
  exit 1
fi
if [[ -z "$CONSOLE_DEVICE_ID" && -n "$CONSOLE_DEVICE_PUBLIC_KEY" ]]; then
  echo "--console-device-id is required when --console-device-public-key is set" >&2
  exit 1
fi
...
log_step() {
  echo "[$(timestamp)] [octogen-bootstrap] $*"
}
...
on_error() {
  ...
  for unit in octogen-desktop.service openclaw-gateway.service octogen-proxy.service octogen-hermes-runner.service caddy.service; do
...
prewarm_browser_profile() {
  ...
  local log_file="/var/log/octogen-browser-prewarm.log"
  local attempt
  ...
  for ((attempt=1; attempt<=attempts; attempt++)); do
    ...
    systemctl restart octogen-desktop.service >/dev/null 2>&1 || true
...
verify_gateway_local_rpc() {
  ...
  for ((attempt=1; attempt<=attempts; attempt++)); do
    if OPENCLAW_MANAGED_TRUSTED_PROXY=1 openclaw gateway probe --json --timeout 5000 >/tmp/octogen-gateway-probe.json 2>/tmp/octogen-gateway-probe.err; then
    ...
    local err_tail=""
    err_tail="$(tail -n 4 /tmp/octogen-gateway-probe.err 2>/dev/null | tr '\n' ' ' | sed 's/[[:space:]]\\{1,\\}/ /g' || true)"
  ...
  tail -n 80 /tmp/octogen-gateway-probe.err 2>/dev/null || true
  log_step "gateway probe json:"
  cat /tmp/octogen-gateway-probe.json 2>/dev/null || true
  return 1
}
...
resolve_gateway_origin_from_template() {
  ...
  rendered="${rendered//\{\{VM_ID\}\}/$instance_id}"
...
prepare_novnc_web_root() {
  ...
  local target_root="/opt/octogen/novnc"
  ...
  local filter_script_path="${active_root}/octogen-error-filter.js"
...
(() => {
  ...
  if (window.__OCTOGEN_NOVNC_ERROR_FILTER === true) {
    return;
  }
  window.__OCTOGEN_NOVNC_ERROR_FILTER = true;
  ...
  const hideKnownNoVncErrorPanels = () => {
    ...
    for (const node of nodes) {
      ...
      panel.setAttribute("data-octogen-hidden-error", "1");
    }
  };
...
script_tag = '<script src="octogen-error-filter.js"></script>'
for name in ("vnc.html", "vnc_lite.html", "vnc_auto.html"):
    html_path = root / name
    if not html_path.exists():
        continue
    source = html_path.read_text(encoding="utf-8")
    if "octogen-error-filter.js" in source:
        continue
    if "</head>" in source:
...
if (source.includes("OCTOGEN_EXTENSION_ERROR_FILTER_V3")) {
  process.exit(0);
}
...
source = source.replace(
  /function shouldIgnoreKnownExtensionInjectionErrors\(event, err\) \{[\s\S]*?\/\/ OCTOGEN_EXTENSION_ERROR_FILTER(?:_V2|_V3)?\n/m,
  "",
);
...
// OCTOGEN_EXTENSION_ERROR_FILTER_V3
`;
...
install_desktop_runtime_files() {
  cat >/opt/octogen/octogen-desktop-runtime.sh <<'EOF_DESKTOP_SCRIPT'
#!/usr/bin/env bash
set -euo pipefail

NEKO_IMAGE="${OCTOGEN_DESKTOP_NEKO_IMAGE:-ghcr.io/cayden970207/openclaw-sandbox-browser:neko}"
NEKO_CONTAINER="${OCTOGEN_DESKTOP_NEKO_CONTAINER:-octogen-neko}"
NEKO_PORT="${OCTOGEN_DESKTOP_NEKO_PORT:-8080}"
NEKO_EPR="${OCTOGEN_DESKTOP_NEKO_EPR:-52000-52100}"
NEKO_PASSWORD="${OCTOGEN_DESKTOP_NEKO_PASSWORD:-octogen}"
NEKO_SCREEN="${OCTOGEN_DESKTOP_NEKO_SCREEN:-1920x1080@30}"
CDP_PORT="${OCTOGEN_DESKTOP_CDP_PORT:-9222}"
...
docker run --rm --name "$NEKO_CONTAINER" \
  ...
  "$NEKO_IMAGE"
EOF_DESKTOP_SCRIPT
  chmod +x /opt/octogen/octogen-desktop-runtime.sh

cat >/etc/octogen-desktop.env <<EOF_DESKTOP_ENV
OCTOGEN_DESKTOP_NEKO_IMAGE=${DESKTOP_NEKO_IMAGE}
OCTOGEN_DESKTOP_NEKO_CONTAINER=${DESKTOP_NEKO_CONTAINER}
OCTOGEN_DESKTOP_NEKO_PORT=${DESKTOP_NEKO_PORT}
OCTOGEN_DESKTOP_NEKO_EPR=${DESKTOP_NEKO_EPR}
OCTOGEN_DESKTOP_NEKO_PASSWORD=octogen
OCTOGEN_DESKTOP_NEKO_SCREEN=1920x1080@30
OCTOGEN_DESKTOP_CDP_PORT=9222
LANG=en_US.UTF-8
LC_ALL=en_US.UTF-8
LANGUAGE=en_US:en
EOF_DESKTOP_ENV
...
install -d -m 0755 /opt/octogen
curl -fsSL "$PROXY_SCRIPT_URL" -o /opt/octogen/octogen-proxy.mjs
chmod +x /opt/octogen/octogen-proxy.mjs

write_state "runner" "running" "installing Hermes runner script"
log_step "installing Hermes runner script from ${HERMES_RUNNER_SCRIPT_URL}"
curl -fsSL "$HERMES_RUNNER_SCRIPT_URL" -o /opt/octogen/octogen-hermes-runner.mjs
chmod +x /opt/octogen/octogen-hermes-runner.mjs
...
if [[ -n "$CONTROL_UI_MANIFEST_URL" ]]; then
  ...
  curl -fsSL "$CONTROL_UI_UPDATER_SCRIPT_URL" -o /opt/octogen/octogen-control-ui-updater.sh
  chmod +x /opt/octogen/octogen-control-ui-updater.sh
  ...
  if CONTROL_UI_UPDATE_OUTPUT="$(
    /opt/octogen/octogen-control-ui-updater.sh \
      --manifest-url "$CONTROL_UI_MANIFEST_URL" \
      --install-root /opt/octogen/control-ui \
      --print-root
  )"; then
  ...
  if [[ -f /opt/octogen/control-ui/current.backend.install-spec ]]; then
    OPENCLAW_INSTALL_TARGET="$(tr -d '\r' </opt/octogen/control-ui/current.backend.install-spec | tr -d '\n')"
  fi
else
...
cat >/etc/octogen-proxy.env <<EOF_ENV
OCTOGEN_PROXY_SHARED_SECRET=${PROXY_SECRET}
OCTOGEN_PROXY_BIND=${PROXY_BIND}
OCTOGEN_PROXY_PORT=${PROXY_PORT}
OCTOGEN_VM_ID=${INSTANCE_ID}
OCTOGEN_INSTANCE_ID=${INSTANCE_ID}
OCTOGEN_TRUSTED_PROXY_USER=${TRUSTED_PROXY_USER}
OCTOGEN_PROXY_LOCAL_GATEWAY_PREFIX=/__octogen/local-gateway
...
OCTOGEN_OPENCLAW_UPSTREAM=http://127.0.0.1:${GATEWAY_PORT}
OCTOGEN_OPENCLAW_LOCAL_ADDRESS=127.0.0.2
OCTOGEN_DESKTOP_UPSTREAM=http://127.0.0.1:${DESKTOP_NEKO_PORT}
OCTOGEN_NOVNC_UPSTREAM=http://127.0.0.1:${DESKTOP_NEKO_PORT}
OCTOGEN_PROXY_CONTROL_PREFIX=${CONTROL_PREFIX}
OCTOGEN_PROXY_DESKTOP_PREFIX=/desktop
OCTOGEN_PROXY_NOVNC_PREFIX=${NOVNC_PREFIX}
OCTOGEN_PROXY_EXPECTED_ISS=octogen-console
OCTOGEN_PROXY_EXPECTED_AUD=openclaw-gateway
OCTOGEN_PROXY_HEALTH_UPSTREAM_TIMEOUT_MS=2000
OCTOGEN_CONTROL_UI_MANIFEST_URL=${CONTROL_UI_MANIFEST_URL}
OCTOGEN_PROXY_SCRIPT_URL=${PROXY_SCRIPT_URL}
OCTOGEN_CONTROL_UI_UPDATER_SCRIPT_URL=${CONTROL_UI_UPDATER_SCRIPT_URL}
OCTOGEN_CONTROL_UI_UPDATER_SCRIPT_PATH=/opt/octogen/octogen-control-ui-updater.sh
OCTOGEN_CONTROL_UI_INSTALL_ROOT=/opt/octogen/control-ui
EOF_ENV
chmod 0600 /etc/octogen-proxy.env

cat >/etc/octogen-runner.env <<EOF_ENV
OCTOGEN_CONSOLE_API_BASE_URL=${CONSOLE_API_BASE_URL}
OCTOGEN_RUNNER_SHARED_SECRET=${PROXY_SECRET}
OCTOGEN_RUNNER_VM_ID=${INSTANCE_ID}
OCTOGEN_RUNNER_INSTANCE_ID=${INSTANCE_ID}
OCTOGEN_HERMES_RUNNER_ID=${INSTANCE_ID}
OCTOGEN_HERMES_RUNNER_MODE=auto
OCTOGEN_HERMES_RUNNER_POLL_SECONDS=10
OCTOGEN_HERMES_SUCCESS_RUNTIME_STATUS=runtime_ready
EOF_ENV
chmod 0600 /etc/octogen-runner.env
...
OCTOGEN_ENABLE_HTTPS="$ENABLE_HTTPS" \
OCTOGEN_CONTROL_UI_ALLOWED_ORIGIN="$CONTROL_UI_ALLOWED_ORIGIN" \
OCTOGEN_CONTROL_UI_ROOT="$CONTROL_UI_ROOT" \
OCTOGEN_CONSOLE_DEVICE_ID="$CONSOLE_DEVICE_ID" \
OCTOGEN_CONSOLE_DEVICE_PUBLIC_KEY="$CONSOLE_DEVICE_PUBLIC_KEY" \
OCTOGEN_PROXY_PORT="$PROXY_PORT" \
OCTOGEN_PROXY_LOCAL_GATEWAY_PREFIX="/__octogen/local-gateway" \
OCTOGEN_TRUSTED_PROXY_USER="$TRUSTED_PROXY_USER" \
node <<'NODE'
...
const proxyPortRaw = String(process.env.OCTOGEN_PROXY_PORT || '18790').trim();
...
const localGatewayPrefixRaw = String(
  process.env.OCTOGEN_PROXY_LOCAL_GATEWAY_PREFIX || '/__octogen/local-gateway',
).trim();
const localGatewayPrefix = (() => {
  if (!localGatewayPrefixRaw) {
    return '/__octogen/local-gateway';
  }
  const normalized = localGatewayPrefixRaw.startsWith('/')
    ? localGatewayPrefixRaw
    : `/${localGatewayPrefixRaw}`;
  return normalized.replace(/\/+$/, '') || '/__octogen/local-gateway';
})();
...
// wiping the in-memory wizard session and forcing users to start over.
	...
	// Octogen Console is a managed trusted-proxy deployment. Keep gateway auth in
...
config.gateway.auth.trustedProxy.requiredHeaders = [
		  'x-octogen-verified',
	  'x-octogen-instance-id',
	  'x-octogen-session-type',
];
...
config.gateway.controlUi.basePath = process.env.CONTROL_PREFIX || '/octogen';
...
config.update =
  config.update && typeof config.update === 'object' && !Array.isArray(config.update)
    ? config.update
    : {};
// Octogen Console manages OpenClaw upgrades at the platform level.
...
config.tools.exec =
  config.tools.exec && typeof config.tools.exec === 'object' && !Array.isArray(config.tools.exec)
    ? config.tools.exec
    : {};
// Octogen Console runs managed gateway/browser flows; keep shell exec in allowlist mode by default
...
const controlUiRoot = (process.env.OCTOGEN_CONTROL_UI_ROOT || '').trim();
if (controlUiRoot) {
  config.gateway.controlUi.root = controlUiRoot;
}
const allowedOrigin = (process.env.OCTOGEN_CONTROL_UI_ALLOWED_ORIGIN || '').trim();
if (allowedOrigin) {
...
// Octogen proxy is the trust boundary for managed browser sessions.
...
const consoleDeviceId = (
  process.env.OCTOGEN_CONSOLE_DEVICE_ID ||
  ''
).trim();
const consoleDevicePublicKey = (
  process.env.OCTOGEN_CONSOLE_DEVICE_PUBLIC_KEY ||
  ''
).trim();
if ((consoleDeviceId && !consoleDevicePublicKey) || (!consoleDeviceId && consoleDevicePublicKey)) {
  throw new Error('console device id/public key must both be set');
}
if (consoleDeviceId && consoleDevicePublicKey) {
  const now = Date.now();
  const deviceScopes = [
    'operator.admin',
    'operator.read',
  ...
  const existing = paired[consoleDeviceId] && typeof paired[consoleDeviceId] === 'object'
    ? paired[consoleDeviceId]
    : {};
  ...
  paired[consoleDeviceId] = {
    ...
    publicKey: consoleDevicePublicKey,
    displayName: existing.displayName || 'octogen-console',
...
${PUBLIC_HOST} {
  @bootstrap path /__octogen/bootstrap
  handle @bootstrap {
    root * ${STATE_DIR}
...
cat >/etc/systemd/system/octogen-desktop.service <<'EOF_DESKTOP_SERVICE'
[Unit]
Description=Octogen Console Desktop Runtime (neko WebRTC)
...
[Service]
Type=simple
EnvironmentFile=-/etc/octogen-desktop.env
ExecStart=/usr/bin/env bash -lc '/opt/octogen/octogen-desktop-runtime.sh'
ExecStopPost=/usr/bin/docker rm -f octogen-neko
Restart=always
RestartSec=5
User=root
WorkingDirectory=/opt/octogen
...
After=network.target docker.service octogen-desktop.service
Requires=docker.service
Wants=octogen-desktop.service
StartLimitIntervalSec=0
...
cat >/etc/systemd/system/octogen-proxy.service <<'EOF_PROXY'
[Unit]
Description=Octogen Console Trusted Proxy
...
[Service]
Type=simple
EnvironmentFile=/etc/octogen-proxy.env
ExecStart=/usr/bin/env bash -lc 'node /opt/octogen/octogen-proxy.mjs'
Restart=always
RestartSec=3
User=root
WorkingDirectory=/opt/octogen
...
cat >/etc/systemd/system/octogen-hermes-runner.service <<'EOF_HERMES_RUNNER'
[Unit]
Description=Octogen Hermes Runtime Runner
After=network-online.target openclaw-gateway.service octogen-proxy.service
Wants=network-online.target openclaw-gateway.service octogen-proxy.service
StartLimitIntervalSec=0

[Service]
Type=simple
EnvironmentFile=/etc/octogen-runner.env
ExecStart=/usr/bin/env bash -lc 'node /opt/octogen/octogen-hermes-runner.mjs'
Restart=always
RestartSec=10
StandardOutput=append:/var/log/octogen-hermes-runner.log
StandardError=append:/var/log/octogen-hermes-runner.log
User=root
WorkingDirectory=/opt/octogen
...
log_step "starting octogen-desktop.service (neko WebRTC)"
systemctl enable octogen-desktop.service
ensure_service_active octogen-desktop.service 4 2
...
write_state "proxy_start" "running" "starting octogen proxy"
log_step "starting octogen-proxy.service"
systemctl enable octogen-proxy.service
ensure_service_active octogen-proxy.service 4 2

write_state "hermes_runner_start" "running" "starting Hermes runner"
log_step "starting octogen-hermes-runner.service"
systemctl enable octogen-hermes-runner.service
ensure_service_active octogen-hermes-runner.service 4 2
...
if ! ensure_port_ready_for_service 127.0.0.1 "$PROXY_PORT" octogen-proxy.service "octogen proxy" 120 1; then
  write_state "failed" "error" "proxy port ${PROXY_PORT} did not become ready in time"
  echo "Octogen proxy port ${PROXY_PORT} did not become ready in time" >&2
  exit 1
fi
if ! ensure_port_ready_for_service 127.0.0.1 "$DESKTOP_NEKO_PORT" octogen-desktop.service "desktop neko" 120 1; then
...
if ! ensure_port_ready_for_service 127.0.0.1 "$PROXY_PORT" octogen-proxy.service "octogen proxy" 45 1; then
  write_state "failed" "error" "proxy became unavailable after browser warm-up"
  echo "Octogen proxy became unavailable after browser warm-up" >&2
  exit 1
fi
...
systemctl is-active octogen-desktop.service
systemctl is-active openclaw-gateway.service
systemctl is-active octogen-proxy.service
systemctl is-active octogen-hermes-runner.service
if [[ "$ENABLE_HTTPS" == "1" ]]; then
  systemctl is-active caddy
fi

echo "OCTOGEN CONSOLE VM bootstrap complete."
